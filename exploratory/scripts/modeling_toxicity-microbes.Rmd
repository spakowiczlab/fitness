...
```{r}
tox.logreg <- read.csv("../data/modelling_toxicity-microbes_longitudinal-ordinal.csv")

tox.logreg %>%
  drop_na(p.value) %>%
  mutate(miclab = ifelse(p.value < 0.05,
                         gsub(".*(\\w__.*)", "\\1",term), NA),
         collab = str_sub(miclab, 1,1)) %>%
  ggplot(aes(x = estimate, y = -log(p.value), label = miclab,
             color = collab)) +
  geom_point() +
  geom_text_repel(show.legend = F) +
  scale_color_viridis_d(breaks = c("f", "g", "s"),
...
ggsave("../figures/volcano_toxicity-mics_longitudinal.png")
```
