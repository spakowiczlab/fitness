...
```{r collect longitudinal inputs}
test <- clin.withdate %>% filter(redcap_repeat_instrument == "" & id %in% baseline.vals$id) %>% select(id, redcap_event_name, contains("carg")) %>%
  select(-contains("timestamp"))
check <- test %>% filter(!is.na(carg_ga_complete))
```

```{r, eval = FALSE}
tmp <- colnames(clin.withdate)
tmp[grepl("ga", tmp)]
```
...
baseline.vals %>%
  ...
  labs(x = "CARG", y = "irAE events") +
  scale_fill_brewer(palette = "Set1", 
                    name = "Toxicity\n\u2265 Grade 3",
                    direction = -1) +
  theme_bw() +
  theme(axis.text.x = element_text())
ggsave("../figures/barplot_carg-tox.png")
```
...
baseline.vals %>%
  ...
  geom_jitter(show.legend = F, height = 0, width = .2) +
  labs(x = "Toxicity \u2265 Grade 3", y = "CARG") +
  scale_fill_brewer(palette = "Set1", 
                    direction = -1) +
  scale_y_continuous(breaks = 1:13) +
  theme_bw()
ggsave("../figures/boxplot_carg-irAE.png")
```
