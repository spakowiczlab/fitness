...
long_plot(sppb.plotin, sppb.mord, "SPPB")
...
big.lolli.ord <- big.lolli.df %>%
  ...
  summarise(max.effect = max(Estimate.scaled)) %>%
  arrange(max.effect) %>%
  select(display.name) %>%
...
```{r}
ggplot(big.lolli.df, aes(x = Estimate.scaled, xend = 0,
                         y = mod.y, yend = mod.y,
...
ggsave("../figures/lollipop_long-fixef_many.svg", height = 3, width = 4)
```