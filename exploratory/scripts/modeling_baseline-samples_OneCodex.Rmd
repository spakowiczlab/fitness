...
promis.barin %>%
  ggplot(aes(x = fct_relevel(taxname, promis.micord), y = estimate, fill = taxlev)) +
  geom_col() +
  scale_fill_viridis(discrete = T, name = "Taxonomy level", breaks = c("f", "g", "s"),
                     labels = c("Family", "Genus", "Species")) +
  ...
  coord_flip() +
  ggsave("../figures/barplot_promis-effects_OC.pdf", height = 4, width = 6)
```
...
sppb.barin %>%
  ...
  coord_flip() +
  ggsave("../figures/barplot_sppb-effects_OC.pdf", height = 4, width = 6)
```
...
fs13.barin %>%
  ...
  coord_flip() +
  ggsave("../figures/barplot_fs13-effects_OC.pdf", height = 4, width = 6)
```