...
patkey <- patkey.raw %>%
  ...
  separate(Name, by = " ", into = c("patient.id", "date")) %>%
...
```{r}
write.csv(clindat.sampkey, file.path(paths$data, "clinical-with-seqids_baseline.csv"), row.names = F)
write.csv(matched.microbes, file.path(paths$data, "matched-microbes_baseline.csv"), row.names = F)
```
